...
class Backgammon:
    ...
    def add_in_house(self, dice):
        ...
        if self.first_dice and self.first_dice == dice:
            ...
            self.second_dice = 0
        return Backgammon(player=new_state.player, table=new_state.table, dice1=new_state.first_dice,
                          dice2=new_state.second_dice, end_pieces_0=new_state.end_pieces_0,
                          end_pieces_1=new_state.end_pieces_1, out_pieces_0=new_state.out_pieces_0,
                          out_pieces_1=new_state.out_pieces_1)
...
def play_game2():
    ...
    while not game.end_game():
        ...
        while game.need_to_put_in_house():
            number_dice = input("Cu ce zar vrei sa intrii in casa?: ")
            game.add_in_house(number_dice)
            print("Tabla dupa ce a intrat cu piesa in casa: ")
            game.print_table()
        while game.can_move():
            position = input("Pozitia de la care vrei sa muti piesa: ")
            number_dice = input("Cu ce zar vrei sa muti piesa?: ")
            game.move(position, number_dice)
            print("Tabla dupa ce a mutat piesa: ")
            game.print_table()
        print("Tabla la final: ")
        game.print_table()
        game.switch_player()
